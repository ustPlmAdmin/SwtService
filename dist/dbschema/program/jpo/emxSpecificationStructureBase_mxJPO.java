 ...
 * @quickreview KIE1 ZUD 28:10:17: IR-466628-3DEXPERIENCER2018x: Unable to promote chapter with customized policy
 ...
 * @quickreview KIE1 ZUD 08:11:2017 : IR-508254-3DEXPERIENCER2018x: R419-STP: While Drag on Drop on structure view Paste as a Sibling option is KO
 */
...
/***********************************************************************************************************
 ...
 2018x   HAT1	  ZUD		06:28:2017  IR-530241-3DEXPERIENCER2018x: R20-FUN067990: "Lock Derived requirement" option should be Removed From the Lock/Unlock option on the Action menu. Methods added: showFieldReserveDerived() showFieldUnReserveDerived().
 *************************************************************************************************************/
                ...
                if (atts[ii].equals("Content Data") || atts[ii].equals("Content Text")) {
                    if (val.length() > 0) {
                        if (atts[ii].equals("Content Text")) {
                            ...
                            val = Base64.getMimeEncoder().encodeToString(val.getBytes("UTF-8"));
                        }
    ...
    public HashMap processXMLMessage(Context context, String[] args)
        ...
        try {
            ...
            if (reseqObjectIdVector.size() > 0) {
                ...
                doEditOperation(context, reseqPasteAboveObjectId, reseqMarkupPasteAboveList, reseqPasteAboveRelId, strParentObjectId, PASTEABOVE, "resequence", recycleRelList, ufList, strRootId);
                ...
                doEditOperation(context, reseqPasteBelowObjectId, reseqMarkupPasteBelowList, reseqPasteBelowRelId, strParentObjectId, PASTEBELOW, "resequence", recycleRelList, ufList, strRootId);
                ...
                doEditOperation(context, reseqPasteAsChildObjectId, reseqMarkupPasteAsChildList, reseqPasteAsChildRelId, strParentObjectId, PASTEASCHILD, "resequence", recycleRelList, ufList, strRootId);
    ...
    private Vector doEditOperation(Context context, ArrayList objIds, StringList markupList, ArrayList relIds, String parObjId, String markup, String operation, StringList recycleRelIds, List<Userfact> ufList, String RootId)
        ...
        try {
            ...
            for (int counter = 0; counter < objectSize; counter++) {
                ...
                DomainObject domObject = DomainObject.newInstance(context, objectIdArray[counter]);
                boolean isTestCase = domObject.isKindOf(context, ReqSchemaUtil.getTestCaseType(context));
                ...
                if (doReconcile.equalsIgnoreCase("true") && !isTestCase) {
...
//Added:14-Nov-09:oep:V6R2010xHF1:Sub/Derive Req Add Existing Enhancement
    ...
    public StringList excludeAllocatedRequirements(Context context, String[] args) throws MatrixException {
        try {
            ...
            MapList mlSpecs = DomainObject.findObjects(context, strTypePattern, null, strWhereExpression, slObjectSelects);
                ...
                if (hasLink) {
                    ...
                    if (statFrom == statusLink.none && statTo == statusLink.none) {
                        ...
                        if (idConnetion[oCount] != null && !"".equals(idConnetion[oCount])) {
                            ...
                            String allocStatusSubValue = "";
                            if (slSubReqFromNames != null && slSubReqFromNames.size() == 0)  //To Test for traceability - append (&&  renderHTML)
                            {
                                rel.getAttributeValue(context, "Link Status");
                        ...
                        if (renderHTML) {
                            ...
                            for (int i = 0; i < arr.size(); i++) {
                                ...
                                String property = "";
                                if ("To".equalsIgnoreCase(fromTo[i]))
                                ...
                                subReqLabel = EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource", context.getLocale(), property);
                                // -- HAT1 ZUD: IR-604654-3DEXPERIENCER2019x --

                                String strUrl = (String) arr.get(i);
                                strHtml += "<a href=\"javascript:displaySubInformations('" + rowIds[oCount] + "','" + subReqLabel + "','" + fromTo[i] + "');\" title=\"" + subReqLabel + "\">";
    ...
    private static String getBothDirIcon(statusLink statFrom, statusLink statTo) {
        switch (statFrom) {
            case Valid: {
                switch (statTo) {
                    case Valid:
                        return ICON_REQUIREMENT_VALID_FROM + "," + ICON_REQUIREMENT_VALID_TO; //ICON_REQUIREMENT_VALID_BOTH;
                    case Invalid:
                        return ICON_REQUIREMENT_VALID_FROM + "," + ICON_REQUIREMENT_INVALID_TO; // ICON_REQUIREMENT_VALID_INVALID_BOTH;
                    case Suspect:
                        return ICON_REQUIREMENT_VALID_FROM + "," + ICON_REQUIREMENT_SUSPECT_TO; //ICON_REQUIREMENT_VALID_SUSPECT_BOTH;
                }
                break;
            }
            ...
            case Invalid: {
                switch (statTo) {
                    case Valid:
                        return ICON_REQUIREMENT_INVALID_FROM + "," + ICON_REQUIREMENT_VALID_TO; //ICON_REQUIREMENT_INVALID_VALID_BOTH;
                    case Invalid:
                        return ICON_REQUIREMENT_INVALID_FROM + "," + ICON_REQUIREMENT_INVALID_TO; //ICON_REQUIREMENT_INVALID_BOTH;
                    case Suspect:
                        return ICON_REQUIREMENT_INVALID_FROM + "," + ICON_REQUIREMENT_SUSPECT_TO;//ICON_REQUIREMENT_INVALID_SUSPECT_BOTH;
                }
                break;
            }
            ...
            case Suspect: {
                switch (statTo) {
                    case Valid:
                        return ICON_REQUIREMENT_SUSPECT_FROM + "," + ICON_REQUIREMENT_VALID_TO; //ICON_REQUIREMENT_SUSPECT_VALID_BOTH;
                    case Invalid:
                        return ICON_REQUIREMENT_SUSPECT_FROM + "," + ICON_REQUIREMENT_INVALID_TO; //ICON_REQUIREMENT_SUSPECT_INVALID_BOTH;
                    case Suspect:
                        return ICON_REQUIREMENT_SUSPECT_FROM + "," + ICON_REQUIREMENT_SUSPECT_TO; // ICON_REQUIREMENT_SUSPECT_BOTH;
                }
                break;
            }
            default:
...
//End::V6R2010xHF1:Requirement Allocation Status Icon
    ...
    // ++ HAT1 ZUD: IR-530241-3DEXPERIENCER2018x fix ++

    /**
     * Returns a boolean value true if Field 'Reserve Derived' is to show in form RMTSpecTreeReserveForm.
     *
     * @param context - the eMatrix <code>Context</code> object
     * @param args    - None
     * @return boolean true - to show field, false - to hide field
     * @throws Exception
     */
    public static boolean showFieldReserveDerived(Context context, String[] args) {
        //By-default 'FieldReserveDerived' will not be available.
        return false;
    }

    /**
     * Returns a boolean value true if Field 'UnReserve Derived' is to show in form RMTSpecTreeUnreserveForm.
     *
     * @param context - the eMatrix <code>Context</code> object
     * @param args    - None
     * @return boolean true - to show field, false - to hide field
     * @throws Exception
     */
    public static boolean showFieldUnReserveDerived(Context context, String[] args) {
        //By-default 'FieldUnReserveDerived' will not be available.
        return false;
    }
    // -- HAT1 ZUD: IR-530241-3DEXPERIENCER2018x fix --
    ...
    public String reorderValidation(Context context, String[] args) throws Exception {
        ...
        if (ReqConstants.REORDER_OP_COPY.equalsIgnoreCase(mode)) {
            for (String rowId : aRowIds) {
                ...
                if (type.equalsIgnoreCase(ReqSchemaUtil.getParameterType(context))) {
                    return EnoviaResourceBundle.getProperty(context, "emxRequirementsStringResource",
                            context.getLocale(), "emxRequirements.PlmParameter.CopyDenied");
                }
            ...
            for (String rowId : aRowIds) {
                ...
                if (pid.equals(newpid)) //re-sequence
                {
                    ...
                    if (newParent.isKindOf(context, ReqSchemaUtil.getTestCaseType(context))) {
                        ...
                        if (!objectToPaste.isKindOf(context, ReqSchemaUtil.getParameterType(context))) {
                            return MessageUtil.getMessage(context, null, "emxRequirements.Reorder.Error.InvalidChild",
                                    new String[]{name + " " + rev, parName + " " + parRev}, null, lang, RESOURCE_BUNDLE_REQUIREMENTS_STR);
                        }
                    }
...
// -- HAT1 ZUD: ReqSpec dependency - enhancements.
    ...
    public void createObjectOwnership(Context context, String[] args) throws Exception {
        String fromId = args[0];
        String toId = args[1];
        String relId = args[2];
        ...
        this.createObjectOwnership(context, fromId, toId, relId, relType, fromType, toType);
    }

    protected void createObjectOwnership(Context context, String fromId, String toId, String relId, String relType, String fromType, String toType) throws Exception {
        String attrPropogateAccessSelect = DomainObject.getAttributeSelect(PropertyUtil.getSchemaProperty(context, "attribute_PropagateAccess"));
        DomainObject parentObj = DomainObject.newInstance(context, fromId);
        String attrPropogateAccess = parentObj.getInfo(context, attrPropogateAccessSelect);
        ...
        DomainObject childObj = DomainObject.newInstance(context, toId);
        String attrAccessType = childObj.getInfo(context, attrAccessTypeSelect);

        if (childObj.isKindOf(context, /* ReqSchemaUtil.getRequirementProxyType(context) */ "Requirement Proxy")) {
            return;
        }

        //special logic when from side object is a folder?
        if ("TRUE".equals(attrPropogateAccess) && "Inherited".equals(attrAccessType)) {
            new emxDomainAccess_mxJPO(context, new String[]{}).createObjectOwnershipInheritance(context, fromId, toId, relId, relType, fromType, toType, null, false);
        }
    ...
    public boolean fixObjectOwnershipInheritanceAfterModifyTo(Context context, String[] args) throws Exception {
        ...
        if (!DomainObject.newInstance(context, toId).isKindOf(context, /* ReqSchemaUtil.getRequirementProxyType(context) */ "Requirement Proxy")) {
            emxDomainAccess_mxJPO jpo = new emxDomainAccess_mxJPO(context, args);
            jpo.deleteObjectOwnershipInheritanceByForce(context, args);
        }

        if (!DomainObject.newInstance(context, newToId).isKindOf(context, /* ReqSchemaUtil.getRequirementProxyType(context) */ "Requirement Proxy")) {
            createObjectOwnership(context, fromId, newToId, relId, relType, fromType, newToType);
        }
        return true;
    }
    ...
    public boolean fixObjectOwnershipInheritanceAfterModifyFrom(Context context, String[] args) throws Exception {
        ...
        createObjectOwnership(context, newFromId, toId, relId, relType, newFromType, toType);
        return true;
    }
